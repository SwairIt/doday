"""Tests for the simplified-mode views (/doday/app/simple/*)."""
...
async def test_simple_today_requires_auth(client: AsyncClient) -> None:
    response = await client.get("/doday/app/simple/today")
...
async def test_simple_today_renders_for_authed_user(
    ...
    response = await logged_in_client.get("/doday/app/simple/today")
    assert response.status_code == 200
    body = response.text
    assert "Doday" in body
    assert "simple" in body
    assert 'href="/doday/app/today"' in body


async def test_simple_inbox_requires_auth(client: AsyncClient) -> None:
    response = await client.get("/doday/app/simple/inbox")
    assert response.status_code == 401
...
async def test_simple_inbox_renders_for_authed_user(
    ...
    response = await logged_in_client.get("/doday/app/simple/inbox")
    assert response.status_code == 200
    body = response.text
...
async def test_simple_add_form_renders(logged_in_client: AsyncClient) -> None:
    response = await logged_in_client.get("/doday/app/simple/add")
    assert response.status_code == 200
    body = response.text
...
async def test_simple_add_submit_creates_task(
    ...
    response = await logged_in_client.post(
        "/doday/app/simple/add",
        data={"title": "Купить молоко (test)", "due_date": ""},
        follow_redirects=False,
    )
    assert response.status_code == 303
    assert response.headers["location"] == "/doday/app/simple/inbox"

    inbox = await logged_in_client.get("/doday/app/simple/inbox")
...
async def test_simple_add_submit_with_due_date_redirects_today(
    ...
    assert response.status_code == 303
    assert response.headers["location"] == "/doday/app/simple/today"
...
async def test_simple_add_empty_title_redirects_back(
    ...
    assert response.status_code == 303
    assert response.headers["location"] == "/doday/app/simple/add"
...
async def test_simple_task_toggle_marks_complete(
    ...
    toggle = await logged_in_client.post(f"/doday/app/simple/task/{task_id}/toggle")
...
async def test_simple_task_delete_returns_empty(
    ...
    delete = await logged_in_client.post(f"/doday/app/simple/task/{task_id}/delete")
    assert delete.status_code == 200
    assert delete.text.strip() == ""

    inbox_after = await logged_in_client.get("/doday/app/simple/inbox")
    assert "Delete test" not in inbox_after.text


async def test_simple_settings_renders(logged_in_client: AsyncClient) -> None:
    response = await logged_in_client.get("/doday/app/simple/settings")
    assert response.status_code == 200
    body = response.text
...
async def test_simple_pages_have_sidebar_nav(logged_in_client: AsyncClient) -> None:
    ...
    for path in [
        "/doday/app/simple/today",
        "/doday/app/simple/inbox",
        "/doday/app/simple/settings",
    ]:
        response = await logged_in_client.get(path)
        assert response.status_code == 200
        assert 'href="/doday/app/simple/today"' in response.text
        assert 'href="/doday/app/simple/inbox"' in response.text
        assert 'href="/doday/app/simple/settings"' in response.text
        # back-to-full link is always available
        assert 'href="/doday/app/today"' in response.text
...
async def test_simple_pages_have_add_button(logged_in_client: AsyncClient) -> None:
    ...
    for path in [
        ...
        if path == "/doday/app/simple/add":
            assert "Отмена" in body
        else:
            assert "/doday/app/simple/add" in body


async def test_simple_toggle_requires_auth(client: AsyncClient) -> None:
    response = await client.post(
        "/doday/app/simple/task/00000000-0000-0000-0000-000000000001/toggle"
    )
    assert response.status_code == 401
