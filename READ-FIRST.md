...
| **Doday ПДД** | `getdoday.ru/pdd/` | (тот же DodayTaskBot) | code-complete, нужен seed | Билеты ПДД АВМ онлайн (free, SEO) + Pro-тренажёр/симулятор экзамена/PDF за Stars через отдельный `pdd_pro` entitlement |
...
**Статус: код готов + тесты написаны, НО НЕ ЗАПУЩЕНО.** Чтобы довести до прода
(юзер локально, правило изоляции БД):
1. `uv run alembic upgrade head` (миграция `0048`).
2. Подготовить датасет АВМ (JSON + картинки в `app/static/pdd/img/`) по
   `docs/pdd-dataset-schema.md` и залить через `app/pdd/seed_load.load_dataset`.
3. Прогнать `uv run pytest tests/test_pdd_*.py tests/test_billing_entitlements.py`
   (модель их написала, но БД-тесты не гонялись в её среде).
4. Реально провести end-to-end Stars-платёж `pdd_pro_3m` (давно висящая
