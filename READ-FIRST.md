...
**Doday ПДД** — новый продукт (2026-05-31), тот же победный паттерн что у
Razbery (free SEO-контент + Pro за Stars), но evergreen-вертикаль без
пересечений: официальные билеты ПДД категории АВМ. Free: билеты/темы/вопросы
с ответами и разбором + интерактивная практика + симулятор экзамена (эфемерно),
всё индексируется. Pro (`/pdd/pro`): тренажёр ошибок, сохранённая история
экзаменов, статистика слабых тем, PDF-сборник (print-страница). **Монетизация —
отдельный `pdd_pro` entitlement** (generic `entitlements`-таблица + поле
`Product.grants_entitlement`), НЕ глобальный `user.tier` — Doday Pro не
открывает ПДД и наоборот. Billing-правка аддитивна (старые продукты не тронуты).
Spec: `docs/superpowers/specs/2026-05-31-doday-pdd-design.md`, plan:
`docs/superpowers/plans/2026-05-31-doday-pdd.md`.
**Статус: ЖИВОЙ на проде.** Миграция `0048` применена авто-деплоем; контент
залит — **800 офиц. вопросов АВМ + 541 картинка** (датасет
`github.com/etspring/pdd_russia` → `scripts/pdd_import_avm.py` →
`app/pdd/seed_data/avm.json` + `app/static/pdd/img/`; засижено через
`app/pdd/seed_cli.py` по SSH на проде). Адаптив проверен Playwright'ом (390/1280),
анимации (card-hover + fade-up) как в студии. Осталось юзеру:
1. Прогнать `uv run pytest tests/test_pdd_*.py tests/test_billing_entitlements.py`
   локально (БД-тесты написаны, но в среде модели тест-БД недоступна).
2. Реально провести end-to-end Stars-платёж `pdd_pro_3m` (давно висящая
   студийная валидация «за Stars платят»).
3. Перегон датасета на новый год ПДД — повторный запуск `scripts/pdd_import_avm.py`
   + `seed_cli` (идемпотентно).
