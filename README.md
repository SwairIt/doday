...
- **[`/lessio`](https://getdoday.ru/lessio)** — **Lessio Web**, booking + CRM for tutors, online trainers, psychologists. Web-first (TG blocked in RU); anon clients book through `/u/<slug>` magic-link, tutors get a cabinet at `/lessio/app/today` (today / calendar / clients / services / schedule / income / settings). Stars products `tutor_pro_*` reserved for future paid tier. Code in [`app/lessio/`](app/lessio/) — `web_router.py` + `cabinet_router.py` + `email.py` + `cron.py` + `csv_export.py` + `og_image.py`. Tables prefixed `lessio_*`. Migrations: `0040_lessio_validation_waitlist.py`, `0041_lessio_telegram_only_users.py`, `0042_lessio_web_expansion.py`. Cron-poll endpoint `POST /api/lessio/cron/dispatch-reminders` (X-Cron-Token) dispatches 24h+1h email reminders.
