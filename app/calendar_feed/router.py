...
def _require_calendar_feed_experiment(user: User) -> None:
    """The .ics calendar feed is experimental — only opt-in users can issue tokens."""
    from app.experiments.service import is_enabled

    if not is_enabled(user, "calendar_feed"):
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            "Включи «Календарь-фид (.ics)» в Настройках → Экспериментальные функции.",
        )
...
async def get_or_create_token(user: RequiredUser, session: DbSession) -> dict[str, str]:
    ...
    _require_calendar_feed_experiment(user)
